...
ExecStart=/usr/bin/docker container run --domainname \${DOMAIN_NAME} --hostname \${HOST_NAME} --name \${CONTAINER_NAME} --rm --publish \${PUBLISH} --volume \${DAEMON_JSON} --volume \${REGISTRY_ROOT} \${REGISTRY}
