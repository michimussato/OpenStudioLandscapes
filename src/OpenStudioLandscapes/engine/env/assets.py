...
def CONFIG(
    ...
    # configs_root = pathlib.Path(EnvVar("OPENSTUDIOLANDSCAPES__CONFIGSTORE_ROOT").get_value(), f"{ASSET_HEADER_BASE_ENV['group_name']}__{'__'.join(ASSET_HEADER_BASE_ENV['key_prefix'])}").expanduser().resolve()
    ...
    config_expanded = expand_dict_vars(
        ...
        kv={
            # "GROUP": ASSET_HEADER_BASE_ENV["group_name"],
            # "KEY": '__'.join(ASSET_HEADER_BASE_ENV["key_prefix"]),
            # # "FEATURE": FEATURE,
            **env,
        },
    )
...
def DOCKER_CONFIG_V2(
    ...
    yield AssetMaterialization(
        ...
        metadata={
            ...
            # "__".join(context.asset_key.path): MetadataValue.md(docker_registry_config.model_dump_json(indent=2, ensure_ascii=True)),
