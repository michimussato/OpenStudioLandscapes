...
def simple_factory_node_exporter(
        ASSET_HEADER,
        compose_scope,
        name,
        ins,
    ...
    @asset(
        **ASSET_HEADER,
        ...
        description=textwrap.dedent(
            ...
            This wrapper is disabled by default.
            To enable it, launch OpenStudioLandscapes with
            - `OPENSTUDIOLANDSCAPES__ATTACH_NODE_EXPORTER_TO_COMPOSE_SCOPE=1`
            
            ---
            
            This Asset implements Prometheus [Node-Exporter](https://hub.docker.com/r/prom/node-exporter/).
            
            - [Source](https://github.com/prometheus/node_exporter)
            
            Node-Exporter runs on every individual host and collects local metrics and publishes
            them via port `9100` (by default).
            You can verify whether metrics are populated by visiting [http://<HOST_IP>:9100/metrics]()
            
            Prometheus can then collect the hosts for their metrics on port 9100.
            
            Configure your Prometheus instances like so:
            ...
            scrape_configs:
            # Add a Node-Exporter Job
            - job_name: node-exporter
              static_configs:
              # Add all hosts to collect metrics from
              - targets: 
                - localhost:9100
                - <IP_ADDRESS>:9100
                - ...
            ...
            A ready made Grafana Dashboard for Node-Exporter can be found here:
            - [https://grafana.com/grafana/dashboards/1860-node-exporter-full/]()
            
            Open Questions:
            - Q: Is Node-Exporter needed or can Alloy be used instead?
    ...
    def _asset(
        ...
        if CONFIG.attach_node_exporter_to_compose_scope:
            ...
            # Node-Exporter needs access to the root file system
            # No relative path conversion required here as
            # the host path is just /
            volumes_dict = {
                "volumes": [
                    f"/:/host:ro,rslave",
                ]
            ...
            service_dict: DockerComposeServiceDefinition = {
                ...
                "container_name": f"node_exporter_container.{_unique_suffix}",
                "restart": DockerComposePolicies.RESTART_POLICY.UNLESS_STOPPED,
                "command": [
                    "--path.rootfs=/host",
                    f"--web.listen-address={CONFIG.grafana_node_exporter_listen_address}",
                ],
                **volumes_dict,
                # Docs refer to host mode but we want to keep the freedom
                # to reassign
                "network_mode": DockerComposePolicies.NETWORK_MODE.HOST,
                # "networks": [],
                # "ports": [
            ...
            unique_node_exporter_service = f"node_exporter_service.{_unique_suffix}"

            service = {
                "services": {
                    unique_node_exporter_service: service_dict,
                },
            }
        ...
        yield AssetMaterialization(
            asset_key=context.asset_key,
            metadata={
                "enabled": MetadataValue.bool(CONFIG.attach_node_exporter_to_compose_scope),
                "service": MetadataValue.md(f"```json\n{json.dumps(service, indent=2, default=str)}\n```"),
                "compose_yaml": MetadataValue.md(f"```yaml\n{compose_yaml}\n```"),
            },
        )
