...
```shell
python3 <(curl --header 'Cache-Control: no-cache, no-store' --silent https://raw.githubusercontent.com/michimussato/OpenStudioLandscapes/refs/heads/main/installer/ubuntu/22.04/install_ubuntu_2204.py) 2>&1 | tee ~/openstudiolandscapes_install_ubuntu_2204_$(date +"%Y-%m-%d_%H-%M-%S").log
```
