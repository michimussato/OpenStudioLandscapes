...
| **Landscape**      | A Landscape is an arbitrary composition of services. It can span across multiple docker compose files providing an arbitrary number of Compose Scopes.                                                                                 |     |
...
| **Registry**       | Registry is a repository where `docker` can push to and pull from. [OpenStudioLandscapesHub](https://github.com/michimussato/OpenStudioLandscapesHub) can provide an easy way to get started with a local registry.                    |     |
| **Compose Scope**  | A Landscape can be broken up into multiple Compose Scopes. For example, a render client will run on a different machine than the render manager - they are part of the same Landscape but belong to separate docker compose files.     |     |
